...
--	END::text AS eez_indicator
	...
	--TOTSEADAYS
	,round(act.v_tmer/24, 2) as totseadays
	
	--TOTKWDAYSATSEA (days*kW)
	,round((act.v_tmer/24)*(b.v_p_cv*0.735499), 3) as totkwdaysatsea
	
	--TOTGTDAYSATSEA (days*GT with GT=K.V with K=0.2+0.02*log(V) ; V volume in m3)
	,round((act.v_tmer/24)*((0.2+0.2*log(b.v_ct_m3))*b.v_ct_m3), 3) as totgtdaysatsea
	
	--TOTFISHDAYS
	,round(act.v_tpec/24, 3) as totfishdays
	
	--TOTKWFISHDAYS
	,round((act.v_tpec/24)*(b.v_p_cv*0.735499), 3) as totkwfishdays
	
	--TOTGTFISHDAYS
	,round((act.v_tpec/24)*((0.2+0.2*log(b.v_ct_m3))*b.v_ct_m3), 3) as totgtfishdays
	
	-- HRSEA
	,round(act.v_tmer, 2) as hrsea
	
	--KWHRSEA
	,round(act.v_tmer*(b.v_p_cv*0.735499), 3) as kwhrsea
	
	--GTHRSEA
	,round(act.v_tmer*((0.2+0.2*log(b.v_ct_m3))*b.v_ct_m3), 3) as gthrsea
